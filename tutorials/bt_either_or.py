...
def create_root() -> py_trees.behaviour.Behaviour:
    """When the FailureIsRunning decorator is applied to a node or 
    subtree, it modifies the behavior such that if the node returns 
    FAILURE, it is treated as RUNNING"""
...
def main() -> None:
    ...
    """
    A pre-tick handler is a user-defined function that is called 
    before the behavior tree is ticked or updated. It allows you to 
    perform custom actions or set up any necessary conditions or 
    state before the tree is evaluated.
    """
    ...
    # if -i or --interavtive given as an argument then only
    #  when key is presse the next line is printed
